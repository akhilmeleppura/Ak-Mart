...
@section('title', 'Return & Refund Management — AK-Mart')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    <div>
        <h4 class="fw-bold mb-1"><i class="bx bx-revision text-primary me-2"></i> Returns & Refund Resolution</h4>
        <p class="text-muted small mb-0">Review customer return requests, process refunds, and manage restocking movements</p>
    </div>
</div>

<div class="card border-0 shadow-sm">
    <div class="card-header border-bottom">
        <h5 class="card-title mb-0">Return Requests Ledger</h5>
    </div>
                ...
                @forelse($requests as $req)
                    <tr>
                        <td>
                            ...
                            <div class="small text-muted">{{ $req->order?->items?->count() ?? 0 }} Items</div>
                        </td>
                        <td>{{ $req->order?->customer?->name ?? 'Customer' }}</td>
                        <td>
                        ...
                        <td class="text-end">
                            <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#editReturnModal{{ $req->id }}">
                                <i class="bx bx-slider me-1"></i> Resolve
                            </button>
                        </td>
                    </tr>
                        ...
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header border-bottom">
                                    <h5 class="modal-title fw-bold">Resolve Return Request #{{ $req->id }}</h5>
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            ...
                                            <select name="status" class="form-select" required>
                                                <option value="pending" {{ $req->status == 'pending' ? 'selected' : '' }}>Pending Review</option>
                                        ...
                                        <div class="mb-3">
                                            <div class="form-check">
                                                ...
                                                <label class="form-check-label fw-semibold" for="restock{{ $req->id }}">
                                                    Restock returned items into inventory (log StockMovement)
                                                </label>
                                            </div>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label fw-semibold">Action Notes / Reason</label>
                                            <textarea name="action_notes" class="form-control" rows="2" placeholder="e.g., Refund issued via original payment gateway..."></textarea>
                                        </div>
                                    </div>
