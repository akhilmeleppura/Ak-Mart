...
@section('title', 'Audit Sheet ' . $stockCount->count_number . ' — AK-Mart')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    <div>
        <a href="{{ route('app-stock-counts') }}" class="btn btn-sm btn-outline-secondary mb-2">
            <i class="bx bx-arrow-back me-1"></i> Back to Audit Sessions
        </a>
        <h4 class="fw-bold mb-0">Stock Audit: {{ $stockCount->count_number }}</h4>
        <small class="text-muted">Type: {{ ucfirst($stockCount->type) }} Count | Location: {{ $stockCount->warehouse?->name ?? 'Global' }} | Status: <span class="badge {{ $stockCount->status === 'reconciled' ? 'bg-success' : 'bg-warning' }}">{{ ucfirst($stockCount->status) }}</span></small>
    </div>
    @if($stockCount->status !== 'reconciled')
        <form action="{{ route('app-stock-counts-reconcile', $stockCount->id) }}" method="POST" onsubmit="return confirm('Reconcile inventory? Live product quantities will be updated to match the counted values.');">
            @csrf
            <button type="submit" class="btn btn-success">
                <i class="bx bx-check-double me-1"></i> Reconcile & Update Inventory
            </button>
        </form>
    @endif
</div>
...
<div class="card shadow-sm border">
    <div class="card-header border-bottom">
        <h5 class="card-title mb-0">Product Count Sheet ({{ $stockCount->items->count() }} SKUs)</h5>
    </div>
            ...
            <tbody>
                @foreach($stockCount->items as $item)
                    <tr>
                        ...
                        <td>
                            @if($stockCount->status !== 'reconciled')
                                <input type="text" id="remarks-{{ $item->id }}" class="form-control form-control-sm" value="{{ $item->remarks }}" placeholder="Discrepancy reason">
                            @else
                                <small class="text-muted">{{ $item->remarks ?: 'No remarks' }}</small>
                            @endif
                        </td>
                        @if($stockCount->status !== 'reconciled')
                            <td>
