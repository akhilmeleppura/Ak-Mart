...
@section('title', 'Reports & Financial Analytics — AK-Mart')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    <div>
        <h4 class="fw-bold mb-1"><i class="bx bx-bar-chart-alt-2 text-primary me-2"></i> Reports & Financial Analytics Suite</h4>
        <p class="text-muted small mb-0">Multi-period sales summaries, full P&L accounting, and deterministic sales forecasting</p>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('app-reports-export-csv') }}" class="btn btn-primary">
            <i class="bx bx-download me-1"></i> Export Sales CSV
        </a>
    </div>
</div>
    ...
    <div class="card-body py-3">
        <form method="GET" action="{{ route('app-reports') }}" class="row g-3 align-items-center">
            ...
            <div class="col-md-4 pt-4">
                <button type="submit" class="btn btn-outline-primary w-100"><i class="bx bx-filter-alt me-1"></i> Update Report Range</button>
            </div>
        </form>
    </div>
</div>
...
<div class="row g-4 mb-4">
    <div class="col-xl-3 col-sm-6">
        <div class="card bg-label-primary p-3 h-100 border">
            <span class="text-muted small">Total Period Sales</span>
            <h3 class="fw-bold text-primary my-1">${{ number_format($totalSales, 2) }}</h3>
            <small class="text-muted"><i class="bx bx-receipt me-1"></i> {{ $totalOrders }} Orders Total</small>
        </div>
    </div>
    <div class="col-xl-3 col-sm-6">
        <div class="card bg-label-success p-3 h-100 border">
            <span class="text-muted small">Estimated Gross Profit</span>
            <h3 class="fw-bold text-success my-1">${{ number_format($grossProfit, 2) }}</h3>
            <small class="text-muted">Gross Margin: ~35%</small>
        </div>
    </div>
    <div class="col-xl-3 col-sm-6">
        <div class="card bg-label-danger p-3 h-100 border">
            <span class="text-muted small">Recorded Store Expenses</span>
...
<div class="nav-align-top mb-4">
    ...
    <div class="tab-content p-0">
        ...
        <div class="tab-pane fade show active" id="tab-pl" role="tabpanel">
            <div class="card border-0 shadow-sm">
                <div class="card-header border-bottom">
                    <h5 class="card-title mb-0">Period Profit & Loss Statement ({{ $startDate }} to {{ $endDate }})</h5>
                </div>
                <div class="card-body p-0">
                    <table class="table table-striped align-middle mb-0">
                        <tbody>
                            <tr class="table-light">
                                <td colspan="2"><strong class="text-heading">1. REVENUE</strong></td>
                            </tr>
                            <tr>
                                <td class="ps-4">Gross Sales Revenue ({{ $totalOrders }} orders)</td>
                                <td class="text-end fw-bold text-primary">${{ number_format($totalSales, 2) }}</td>
                            </tr>
                            <tr class="table-light">
                                <td colspan="2"><strong class="text-heading">2. COST OF GOODS SOLD (COGS)</strong></td>
                            </tr>
                            <tr>
                                <td class="ps-4">Estimated Wholesale Procurement Cost (~65%)</td>
                            ...
                            <tr class="table-light">
                                <td colspan="2"><strong class="text-heading">3. OPERATIONAL EXPENSES</strong></td>
                            </tr>
                            <tr>
                                <td class="ps-4">Store Utilities, Rent & Miscellaneous (Recorded Expenses)</td>
        ...
        <div class="tab-pane fade" id="tab-forecast" role="tabpanel">
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-header border-bottom d-flex justify-content-between align-items-center">
                    <div>
                        <h5 class="card-title mb-0">Deterministic 7-Day & 30-Day Sales Forecast</h5>
                        <small class="text-muted">Based on moving-average historical daily trends and day-of-week seasonality factors</small>
                    </div>
                    <span class="badge bg-label-info">Estimate / Forecast Model</span>
                </div>
                <div class="card-body">
                    <div class="row g-4 mb-4">
                        <div class="col-md-6">
                            <div class="p-3 border rounded bg-light text-center">
                                <span class="text-muted small">Projected 7-Day Revenue</span>
                                <h3 class="text-primary fw-bold my-1">${{ number_format($forecast7DayTotal, 2) }}</h3>
                                <small class="text-muted">7-Day Sum Estimate</small>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="p-3 border rounded bg-light text-center">
                                <span class="text-muted small">Projected 30-Day Revenue</span>
                                <h3 class="text-success fw-bold my-1">${{ number_format($forecast30DayTotal, 2) }}</h3>
                                <small class="text-muted">30-Day Trend Estimate</small>
                            </div>
                        </div>
                    </div>

                    <h6 class="fw-bold mb-3">7-Day Day-by-Day Forecast Breakdown</h6>
        ...
        <div class="tab-pane fade" id="tab-products" role="tabpanel">
            <div class="card border-0 shadow-sm">
                <div class="card-header border-bottom">
                    <h5 class="card-title mb-0">Top-Selling Products by Volume & Revenue</h5>
                </div>
                <div class="table-responsive">
                        ...
                        <tbody>
                            @forelse($topSellingItems as $item)
                                <tr>
                                    ...
                                    <td><span class="badge bg-label-primary fs-6">{{ $item->total_qty }} Units</span></td>
                                    <td class="fw-bold text-success">${{ number_format($item->total_revenue, 2) }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="text-center py-4 text-muted">No sales items recorded in period.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="tab-pane fade" id="tab-purchases" role="tabpanel">
            <div class="card border-0 shadow-sm">
                <div class="card-header border-bottom d-flex justify-content-between align-items-center">
                    <h5 class="card-title mb-0">Supplier Procurement History</h5>
                    <span class="badge bg-label-primary">Total: ${{ number_format($totalPurchasesAmount, 2) }}</span>
                </div>
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        <thead class="table-light">
                        ...
                        <tbody>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center py-4 text-muted">No purchase orders on record.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
