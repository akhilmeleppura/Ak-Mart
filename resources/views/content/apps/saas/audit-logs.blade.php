...
<div class="card">
    <div class="card-header border-bottom">
        <h5 class="card-title mb-0">System Audit Logs</h5>
        <p class="text-muted small mb-0">Tracking all administrative and sensitive actions across the platform.</p>
    </div>
                ...
                <tbody>
                    @forelse($logs as $log)
                        <tr>
                            ...
                            <td>
                                @if($log->new_values)
                                    <button class="btn btn-xs btn-outline-secondary" data-bs-toggle="collapse" data-bs-target="#logDetails{{ $log->id }}">Details</button>
                                @endif
                            </td>
                        </tr>
                        @if($log->new_values)
                        <tr class="collapse" id="logDetails{{ $log->id }}">
                            <td colspan="6" class="bg-light p-4">
                                <div class="row">
                                    <div class="col-md-6">
                                        <h6 class="small fw-bold">Changes:</h6>
                                        <pre class="small mb-0">{{ json_encode($log->new_values, JSON_PRETTY_PRINT) }}</pre>
                                    </div>
                                    <div class="col-md-6">
                                        <h6 class="small fw-bold">User Agent:</h6>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-10 text-muted">No audit logs found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
