...
@section('title', __('Supplier Management') . ' — AK-Mart')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="fw-bold mb-1"><i class="bx bx-truck text-primary me-2"></i> {{ __('Supplier Management') }}</h4>
        <p class="text-muted small mb-0">{{ __('Manage vendor contact details, balances, and purchase relationships') }}</p>
    </div>
    <button class="btn btn-ak-primary" data-bs-toggle="modal" data-bs-target="#addSupplierModal">
        <i class="bx bx-plus me-1"></i> {{ __('Add Supplier') }}
    </button>
</div>
                ...
                <tbody>
                    ...
                    <tr>
                        <td colspan="7" class="text-center py-4 text-muted">
                            ...
                            {{ __('No suppliers found. Click "Add Supplier" to register one.') }}
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div class="modal-content">
            <form action="{{ route('app-suppliers-store') }}" method="POST">
                ...
                <div class="modal-header border-bottom">
                    <h5 class="modal-title fw-bold">{{ __('Add New Supplier') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label fw-semibold">{{ __('Supplier Name') }} *</label>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="address" class="form-control" rows="2" placeholder="{{ __('Full address...') }}"></textarea>
                    </div>
                </div>
                <div class="modal-footer border-top">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ __('Cancel') }}</button>
                    <button type="submit" class="btn btn-ak-primary">{{ __('Save Supplier') }}</button>
                </div>
            </form>
        </div>
    </div>
