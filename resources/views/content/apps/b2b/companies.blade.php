...
@section('title', 'B2B Wholesale Accounts — AK-Mart')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    <div>
        <h4 class="fw-bold mb-1"><i class="bx bx-briefcase-alt text-primary me-2"></i> B2B Wholesale Accounts</h4>
        <p class="text-muted small mb-0">Manage corporate accounts, buyer hierarchies, credit limits, net terms, and negotiated wholesale price tiers</p>
    </div>
    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalAddB2bCompany">
        <i class="bx bx-plus me-1"></i> Register B2B Company
    </button>
</div>
...
<div class="row g-4 mb-4">
    <div class="col-sm-6 col-xl-4">
        <div class="card p-3 border shadow-sm">
            <span class="text-muted small">Active B2B Companies</span>
            <h3 class="fw-bold text-primary my-1">{{ $companies->count() }}</h3>
            <small class="text-muted">Corporate accounts</small>
        </div>
    </div>
    <div class="col-sm-6 col-xl-4">
        <div class="card p-3 border shadow-sm">
            <span class="text-muted small">Total Credit Lines Extended</span>
            <h3 class="fw-bold text-success my-1">${{ number_format($totalCreditExtended, 2) }}</h3>
            <small class="text-muted">Approved company limits</small>
        </div>
    </div>
    <div class="col-sm-6 col-xl-4">
        <div class="card p-3 border shadow-sm">
            <span class="text-muted small">Outstanding B2B Receivables</span>
            <h3 class="fw-bold text-warning my-1">${{ number_format($totalOutstandingBalance, 2) }}</h3>
            <small class="text-muted">Invoiced balance</small>
        </div>
    </div>
</div>
...
<div class="card shadow-sm border">
    <div class="card-header border-bottom">
        <h5 class="card-title mb-0">Registered Corporate Accounts</h5>
    </div>
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="8" class="text-center py-4 text-muted">No B2B accounts registered. Click 'Register B2B Company' above.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="modal-content">
            <form action="{{ route('app-b2b-companies-store') }}" method="POST">
                ...
                <div class="modal-header border-bottom">
                    <h5 class="modal-title fw-bold"><i class="bx bx-briefcase text-primary me-1"></i> Register B2B Company</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-2 mb-3">
                        <div class="col-6">
                            <label class="form-label fw-bold">Tax ID / GSTIN</label>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="billing_address" class="form-control" rows="2" placeholder="Official registered billing address"></textarea>
                    </div>
                </div>
                <div class="modal-footer border-top">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-primary">Save B2B Account</button>
                </div>
            </form>
        </div>
    </div>
