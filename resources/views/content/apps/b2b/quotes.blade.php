...
@section('title', 'B2B Quotes & Custom Estimates — AK-Mart')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    <div>
        <h4 class="fw-bold mb-1"><i class="bx bx-file-blank text-primary me-2"></i> B2B Quotes & Estimates</h4>
        <p class="text-muted small mb-0">Review custom RFQs, calculate bulk tiered discounts, and approve corporate quote estimates</p>
    </div>
    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalCreateQuote">
        <i class="bx bx-plus me-1"></i> Create Quote Request
    </button>
</div>
...
<div class="card shadow-sm border">
    <div class="card-header border-bottom">
        <h5 class="card-title mb-0">Quote Requests & Negotiations</h5>
    </div>
            ...
            <tbody>
                @forelse($quotes as $q)
                    <tr>
                        ...
                        <td>{{ $q->company?->name ?? 'Direct B2B' }}</td>
                        ...
                        <td>
                            @if($q->status === 'submitted')
                                <form action="{{ route('app-b2b-quotes-status', $q->id) }}" method="POST" class="d-inline">
                                    ...
                                    <button type="submit" class="btn btn-sm btn-success"><i class="bx bx-check"></i> Approve</button>
                                </form>
                                <form action="{{ route('app-b2b-quotes-status', $q->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    <input type="hidden" name="status" value="rejected">
                                    <button type="submit" class="btn btn-sm btn-outline-danger"><i class="bx bx-x"></i> Reject</button>
                                </form>
                            @endif
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" class="text-center py-4 text-muted">No quote requests recorded. Click 'Create Quote Request' above.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="modal-content">
            <form action="{{ route('app-b2b-quotes-store') }}" method="POST">
                ...
                <div class="modal-header border-bottom">
                    <h5 class="modal-title fw-bold">Create B2B Quote Request</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label fw-bold">Select Corporate Client <span class="text-danger">*</span></label>
                    ...
                    <div class="row g-2 mb-3">
                        <div class="col-6">
                            <label class="form-label fw-bold">Quantity (MOQ)</label>
                            <input type="number" name="items[0][qty]" class="form-control" value="25" min="1" required>
                        </div>
                        <div class="col-6">
                            <label class="form-label fw-bold">Negotiated Discount %</label>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="notes" class="form-control" rows="2" placeholder="e.g. Free pallet delivery included"></textarea>
                    </div>
                </div>
